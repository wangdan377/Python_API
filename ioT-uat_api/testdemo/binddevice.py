import requests
import json


header={
    'Authorization':'bearer eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9..aZTWNmcZPdbQN_DXxzyBjhzic0vtYN187n1z07dBmS8',
    'Content-type':'application/json'
}

for i in range(0,10):
    response1=requests.get(url='https://mobileuat.utcook.com/iotapp/device/queryBatchInfo',headers=header,params={'productKey':'a16zn1PUREs','applyId':'1398677','pageSize':'100','pageNumber':i})
    res=json.loads(response1.text)
    device=[]
    for n in range(0,100):
        device.append(res['content'][n]['deviceName'])
        print(device)
        param={
                "driverId":"54a69c88583045619c262a6bbef8305f",
                "instanceId":"ATzpRxLPPiWgaL8cD6AA",
                "deviceKeys":[{
                    "deviceName":device[n],
                    "productKey":"a16zn1PUREs"
                }]
        }
        response= requests.post(url='https://mobileuat.utcook.com/iotapp/edge/batchBindDeviceToEdgeInstanceWithDriver',headers=header,data=json.dumps(param,separators=(',',':')))
        print("------"+str(n))
        print(json.dumps(param))
        print(response.text)