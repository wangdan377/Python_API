...
log = Logger()
send_mail = SendEmail(
        username='Linda_aha@163.com',
        passwd='AELAEXIZUSLMUSKE',
        recv=['1065913054@qq.com'],
        title='API自动化测试报告邮件',
        content='测试发送邮件',
        file=r'D:\Python_API\API_smiple_complete\interfaceTest\result\report.html',
        ssl=True,
    )
...
class AllTest:#定义一个类AllTest
    ...
    def set_case_list(self):
        ...
        for value in fb.readlines():        #readlines()读取所有的行
            data = str(value)
            if data != '' and not data.startswith("#"):# 如果data非空且不以#开头     #startswith("#") 以什么开头的
